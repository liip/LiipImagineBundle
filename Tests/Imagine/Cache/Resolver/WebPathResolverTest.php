...
class WebPathResolverTest extends TestCase
{
    ...
    public function testComposeSchemaHostAndFileUrlOnResolve()
    {
        ...
        $pathResolver->method("getFileUrl")
            ...
            ->willReturn("aCachePrefix/aFilter/aPath");
    ...
    public function testDumpBinaryContentOnStore()
    {
        ...
        $filesystemMock
            ->expects($this->once())
            ->method('dumpFile')
            ->with($this->equalTo($fileUrl), $this->equalTo($fileContent));
    ...
    public function testRemoveCacheForSomePathsAndFilterOnRemove()
    {
        ...
        $filesystemMock
            ->expects($this->at(0))
            ->method('remove')
            ->with(sprintf("%s/%s/%s", $cacheRoot, $filter, $pathOne));
        $filesystemMock
            ->expects($this->at(1))
            ->method('remove')
            ->with(sprintf("%s/%s/%s", $cacheRoot, $filter, $pathTwo));
